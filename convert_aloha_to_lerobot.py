    ...
    try:
        with h5py.File(hdf5_file, 'r') as f:
            ...
            for group_name in ['task_info', 'metadata', 'info']:
                if group_name in f:
                    group = f[group_name]
                    if 'task' in group.attrs:
                        task_name = group.attrs['task']
                        if isinstance(task_name, bytes):
                            task_name = task_name.decode('utf-8')
                        return task_name
                    if 'name' in group.attrs:
                        task_name = group.attrs['name']
                        if isinstance(task_name, bytes):
                            task_name = task_name.decode('utf-8')
                        return task_name
    ...
    for dataset_folder in sorted(parent_dir.iterdir()):
        if dataset_folder.is_dir():
            ...
            if not task_string or task_string == "default task":
                ...
                actual_task_string = task_string
            ...
            with open(out_dir / "meta/tasks.jsonl", "w") as f:
                f.write(json.dumps({"task_index": 0, "task": actual_task_string}) + "\n")

            print(f"✅ Done: {dataset_folder.name} → {episode_count} episodes, task: '{actual_task_string}'\n")
...
def main():
    ...
    parser.add_argument('--task_string', type=str, default=None, help="Task name or description. If not provided, will auto-extract from HDF5 files.")
    ...
    print(f"🚀 Starting ALOHA to LeRobot conversion")
    print(f"📁 Input directory: {args.input_dir}")
    print(f"📁 Output directory: {args.output_dir}")
    print(f"📝 Task extraction: {'Auto from HDF5 files' if not args.task_string else f'Manual: {args.task_string}'}")
    print("=" * 60)
